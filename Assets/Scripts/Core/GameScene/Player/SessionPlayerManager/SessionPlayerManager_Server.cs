...
public class SessionPlayerManager_Server {
    ...
    public SessionPlayerManager_Server() {
    ...
    public void AddWinScore( ulong winner ) {
        ...
        } else { Debug.LogWarning( $"[{nameof( SessionPlayerManager_Server )}] Winner ID not fount in dictionary." ); }
    }
    ...
    public void AddLivePlayer( ulong id, GameObject player ) {
        if ( !LivePlayers.Keys.Contains( id ) ) {
            ...
            Debug.LogWarning( $"[{nameof( SessionPlayerManager_Server )}] Player already in LivePlayers list." );
        }
    }
    ...
    public void RemoveLivePlayer( ulong id ) {
        if ( LivePlayers.Remove( id ) ) {
            ...
            Debug.LogWarning( $"[{nameof( SessionPlayerManager_Server )}] Player not found in LivePlayers list." );
        }
    }
    ...
    public void AddDeadPlayer( ulong id, GameObject playerObj ) {
        if ( !DeadPlayers.Keys.Contains( id ) ) {
            ...
            Debug.LogWarning( $"[{nameof( SessionPlayerManager_Server )}] Player already in DeadPlayers list." );
        }
    }
    ...
    public void RemoveDeadPlayer( ulong id ) {
        if ( DeadPlayers.Remove( id ) ) {
            ...
            Debug.LogWarning( $"[{nameof( SessionPlayerManager_Server )}] Player not found in DeadPlayers list." );
        }
    }
