...
public class MapService_Server : MonoBehaviour {
    [SerializeField] private List<GameMap_Data> _maps;
    private List<int> _usedSpawnPosition = new();

    private GameMap_Data _currentMap = null;
    ...
    public void DespawnMap() {
        if ( _currentMap != null ) {
            ...
            Debug.LogWarning( $"[{nameof( MapService_Server )}] No map to despawn." );
        }
    }
