...
public class PlayerManager_Server : IDisposable {
    private PlayerFactory_Server _playerFactory;
    ...
    [Inject]
    public PlayerManager_Server( PlayerFactory_Server playerFactory ) {
        ...
        } else { Debug.LogError( $"[{nameof( PlayerManager_Server )}] NetworkManager is null!" ); }
    ...
    private void OnClientDisconnected( ulong clientId ) {
        Debug.Log( $"[{nameof( PlayerManager_Server )}] {nameof( OnClientDisconnected )} Player ID: {clientId}" );
    ...
    private void RegisterPlayer( NetworkPlayerData playerData ) {
        if ( !RegistredPlayers.ContainsKey( playerData.NetID ) ) {
            Debug.Log( $"[{nameof( PlayerManager_Server )}] Player: {playerData.NetID} registred" ); ;
            RegistredPlayers.Add( playerData.NetID, playerData );
            OnRegistredPlayersUpdated?.Invoke();

        } else {
            Debug.LogWarning( $"[{nameof( PlayerManager_Server )}] Player: {playerData.NetID} is already registred!" );
        }
    }
    ...
    private void UnRegisterPlayer( NetworkPlayerData playerData ) {
        if ( RegistredPlayers.ContainsKey( playerData.NetID ) ) {
            ...
            OnRegistredPlayersUpdated?.Invoke();
            Debug.Log( $"[{nameof( PlayerManager_Server )}] Player: {playerData.NetID} unregistred" ); ;

        } else {
            Debug.LogWarning( $"[{nameof( PlayerManager_Server )}] Player: {playerData.NetID} is not registred!" );
        }
    }
