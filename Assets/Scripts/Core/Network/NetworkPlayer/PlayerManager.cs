...
public class PlayerManager : IDisposable {
    ...
    public PlayerManager( PlayerFactory playerFactory ) {
        ...
        } else { Debug.LogError( $"[{nameof( PlayerManager )}] NetworkManager is null!" ); }
    ...
    private void OnClientDisconnected( ulong clientId ) {
        Debug.Log( $"[{nameof( PlayerManager )}] {nameof( OnClientDisconnected )} Player ID: {clientId}" );
    ...
    private void RegisterPlayer( NetworkPlayerData playerData ) {
        if ( !RegistredPlayers.ContainsKey( playerData.NetID ) ) {
            Debug.Log( $"[{nameof( PlayerManager )}] Player: {playerData.NetID} registred" ); ;
            RegistredPlayers.Add( playerData.NetID, playerData );
            OnRegistredPlayersUpdated?.Invoke();

        } else {
            Debug.LogWarning( $"[{nameof( PlayerManager )}] Player: {playerData.NetID} is already registred!" );
        }
    }
    ...
    private void UnRegisterPlayer( NetworkPlayerData playerData ) {
        if ( RegistredPlayers.ContainsKey( playerData.NetID ) ) {
            ...
            OnRegistredPlayersUpdated?.Invoke();
            Debug.Log( $"[{nameof( PlayerManager )}] Player: {playerData.NetID} unregistred" ); ;

        } else {
            Debug.LogWarning( $"[{nameof( PlayerManager )}] Player: {playerData.NetID} is not registred!" );
        }
    }
